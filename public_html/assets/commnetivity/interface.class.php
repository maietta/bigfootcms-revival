...
class screen {
	...
	if ($label != "") {
		$xhtml  = '<label for="'.$elements['name'].'">'.$label.'';
	}
	$xhtml .= '<input ';
	while ( list ( $name, $value ) = each ( $elements ) ) {
		$xhtml .= $name . '="' . $value . '" ';
	}
	$xhtml .= 'value="'.$default_value.'"></label>';
		if ($spantext != "") {
    ...
    function make_select($label="", $field_name, $default_value, $options) {
		...
		if ($label != ""){
			$xhtml  = '<label for="'.$field_name.'">'.$label.'' . "\n";
		}
		$xhtml .= '<select name="'. $field_name. '">' . "\n";
		while ( list($option_value, $option_name ) = each ( $options ) ) {
