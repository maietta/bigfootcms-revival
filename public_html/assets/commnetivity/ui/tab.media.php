...
function initialize($framework, $ui, $params){
    ...
    $xhtml .= '<li class="commnetivity_ui_left"><a href="/commnetivity/tab/media/images">Images/Photos</a></li>';
    $xhtml .= '<li class="commnetivity_ui_middle"><a href="/commnetivity/tab/media/audio">Audio</a></li>';
    $xhtml .= '<li class="commnetivity_ui_right"><a href="/commnetivity/tab/media/video">Video</a></li>';
    //$xhtml .= '<li class="commnetivity_ui_right"><a href="/commnetivity/tab/media/streams">Live Streams</a></li>';
    $xhtml .= '<li class="commnetivity_ui_seperator"></li>';
    $xhtml .= '<li class="commnetivity_ui_left"><a href="/commnetivity/tab/media/players">Player/Presentation</a></li>';
    $xhtml .= '<li class="commnetivity_ui_right"><a href="/commnetivity/tab/media/galleries">Photo Galleries</a></li>';
    //$xhtml .= '<li class="commnetivity_ui_right"><a href="/commnetivity/tab/media/feeds">RSS & XML Feeds</a></li>';
    //$xhtml .= '<li class="commnetivity_ui_seperator"></li>';
   // $xhtml .= '<li class="commnetivity_ui_help"><a href="/commnetivity/tab/media/help" class="help">?</a></li>';
    $xhtml .= '</ul>';
...
function details($framework, $ui, $params) {
    switch (SCREEN) { // switch to specific details function for each media type.
        case "images" : $xhtml = image_details($framework, $ui, $params); break;
        case "audio" : $xhtml = avdetails($framework, $ui, $params); break;
        case "video" : $xhtml = avdetails($framework, $ui, $params); break;
        case "players" : $xhtml = player_details($framework, $ui, $params); break;
		case "player" : $xhtml = player_details($framework, $ui, $params); break;
        case "galleries" : $xhtml = gallery_details($framework, $ui, $params); break;
...
function preview($framework, $ui, $params) {
    switch(SCREEN) { // Detecting screen from rail
        case 'images' : $xhtml = images_cms_preview($framework, $ui, $params); break;
        case 'audio' : $xhtml = $ui->make_audio_player($framework, $ui, $params); break;
        case 'video' : $xhtml = $ui->make_video_player($framework, $ui, $params); break;
        case 'other' : $xhtml = $ui->other_cms_preview($framework, $ui, $params); break;
        case 'players' : $xhtml = $ui->make_video_player($framework, $ui, $params); break;
        case 'galleries' : $xhtml = $ui->make_galleries($framework, $ui, $params); break;
        case 'feeds' : $xhtml = $ui->make_feeds($framework, $ui, $params); exit; break;
    }
   return $xhtml;
}

function audio($framework, $ui, $params) { //Field named is this... so we direct that field name to use another function.
	return avpanel($framework, $ui, $params);
}
function video($framework, $ui, $params) { // Same thing as audio. Field name is function name for normal screens.... RPC's are different.
	return avpanel($framework, $ui, $params);
}
...
function count_files_by_group($group) {
	return mysql_num_rows(mysql_query("SELECT * FROM `commnetivity_media` WHERE `group` = '".$group."';"));
}

function players($framework, $ui, $params) { // formally the audio and video functions. Now both in one.
	$record = RECORD;
	$panel = PANEL;
	if ( ACTION != "" ) {
	...
	if (SCREEN == 'search') {
		$sql = ( defined('RECORD') ) ? "SELECT * FROM `commnetivity_presentation` WHERE `group` != '' LIMIT 15;"
									 : "SELECT * FROM `commnetivity_presentation` WHERE `LIMIT 15;";
	} else {	
...
<div id="commnetivity_windows">
	<div id="commnetivity_details"><div class="head">Please select a presentation to begin working.</div></div>
	<div id="commnetivity_preview"></div>
</div>
<style>
.commnetivity_leftnav  a {
	background:transparent url(/icons/folder.png) center left no-repeat;
	display:inline-block;
	padding-left:20px;
	line-height:18px;
}
.commnetivity_leftnav a[href$='.mp3'], a[href$='.rar'], a[href$='.gzip'] {
...
$javascript .= <<<EOF
	...
	$('div[id=commnetivity_actions]').html('<a class="prev">&laquo; Back</a><a class="next">More results &raquo;</a>');
...
function player_details($framework, $ui, $params) {
	...
	//$xhtml .= '<div id="commnetivity_toolbar"><strong style="text-align: center; font-size: 14px;">Once your media is uploaded. Assign it a group, and then visit this screen to customize a dedicated player.</strong></div>';
    ...
    $presentation = mysql_query("SELECT * FROM `commnetivity_presentation` WHERE `group` = '".$group."' LIMIT 1;");
    ...
    if ( mysql_num_rows($presentation) == 1 ) { // YES, THERE IS PRESENTATION DATA IN TABLE.
       ...
       // if ( !isset($flash_vars->screencolor) ) { $flash_vars->screencolor = "000000"; }
		...
		$xhtml .= '<ul>';
		$xhtml .= '<div class="head" style="background-image: url(/assets/commnetivity/ui/images/whitetransparent.png); color: #000; font-size: 15px; padding: 0px; margin: 0px; vertical-align: bottom;">Colors</div>';
        ...
        $xhtml .= '<li title="The screen color of the player">'.$ui->make_input("Screen", array("name"=>"screencolor","id"=>"screencolor", "maxlength"=>"6", "size"=>"6"), $flash_vars->screencolor).'</li>';
				...
				$xhtml .= '<div class="head" style="background-image: url(/assets/commnetivity/ui/images/whitetransparent.png); color: #000; font-size: 15px; padding: 0px; margin: 0px; vertical-align: bottom;">Dimensions</div>';
		...
		$xhtml .= '<ul>';
		$xhtml .= '<div class="head" style="background-image: url(/assets/commnetivity/ui/images/whitetransparent.png); color: #000; font-size: 15px; padding: 0px; margin: 0px; vertical-align: bottom;">Layout</div>';
		...
		$xhtml .= '<div class="head" style="background-image: url(/assets/commnetivity/ui/images/whitetransparent.png); color: #000; font-size: 15px; padding: 0px; margin: 0px; vertical-align: bottom;">Behavior</div>';
    	...
    	$results = mysql_query("SELECT * FROM `commnetivity_media` WHERE `group` = '".$group."'");
	    if ( mysql_num_rows($results) > 0 ) {
			$xhtml .= '<div class="head" style="background-image: url(/assets/commnetivity/ui/images/greentransparent.png); color: white; font-size: 13px;">What\'s in this group?</div>';
			$video_extentions = $framework->get_extentions_from_mimetype("video/%");
...
EOF;
//	$xhtml = '<style>#commnetivity_leftnav_container {height: 80px;}</style>';
	return array("xhtml"=>$xhtml, "javascript"=>"$javascript");
}

function image_details($framework, $ui, $params) {//called from details();
    $results = mysql_query("SELECT * FROM `commnetivity_media` WHERE `id` = '".RECORD."' LIMIT 1;");
    if ( mysql_num_rows($results) == 1 ) {
        $result = mysql_fetch_object($results);
		$xhtml .= '<div id="commnetivity_toolbar">';
		$xhtml .= '	<a href="/commnetivity/tab/media/images/preview/'.RECORD.'" id="cmspreview" class="commpreview">Show preview</a>';
		$xhtml .= '	<a href="/commnetivity/tab/media/images/preview/'.RECORD.'" id="cmspreviewx" class="commpreview">Hide preview</a>';
		$xhtml .= '</div>';
		$xhtml .= '<ul>';
...
function avdetails($framework, $ui, $params) { //called from details();
    $results = mysql_query("SELECT * FROM `commnetivity_media` WHERE `id` = '".RECORD."' LIMIT 1;");
    if ( mysql_num_rows($results) == 1 ) {
            $result = mysql_fetch_object($results);
			$xhtml .= '<div id="commnetivity_toolbar">';
			$xhtml .= '	<a href="/commnetivity/tab/media/'.SCREEN.'/preview/'.RECORD.'" id="cmspreview" class="commpreview">Run in media player</a>';
			$xhtml .= '	<a href="/commnetivity/tab/media/'.SCREEN.'/preview/'.RECORD.'" id="cmspreviewx" class="commpreview">Hide media player</a>';
			$xhtml .= '</div>';
...
function avpanel($framework, $ui, $params) { // formally the audio and video functions. Now both in one.
	$record = RECORD;
	$panel = PANEL;
	if ( ACTION != "" ) {
		// use switch(ACTION) here.
	...
	$xhtml .= '<div id="commnetivity_leftnav_container">';
	if (SCREEN == 'search') {
		$sql = ( defined('RECORD') ) ? "SELECT * FROM `commnetivity_media` WHERE `extention` IN('".implode("', '", $extentions)."') AND `orig_filename` LIKE '%".RECORD."%' LIMIT 15;"
									 : "SELECT * FROM `commnetivity_media` WHERE `extention` IN('".implode("', '", $extentions)."') LIMIT 15;";
	} else {	
		$xhtml .= '<div id="commnetivity_leftnav_search"><input name="commnetivity_leftnav_search" value="Search"></div>';
	    $xhtml .= '<div id="commnetivity_leftnav" class="scrollable vertical"><div class="items">'."\n";
		$sql = "SELECT * FROM `commnetivity_media` WHERE `extention` IN('".implode("', '", $extentions)."') ";
	}
	$results = mysql_query($sql);
	$counter = 0;
    if ( mysql_num_rows($results) > 0 ) {
...
<div id="commnetivity_windows">
	<div id="commnetivity_details">Please select a file to begin working.</div>
	<div id="commnetivity_preview"></div>
</div>
EOF;
//$xhtml .= '</textarea>';
...
function files_by_association($tag, $query) {
	$results = mysql_query("SELECT * FROM `commnetivity_media` WHERE `".$tag."` = '".$query."';");
	$files = array();
	if (mysql_num_rows($results)>0) {
...
function streams($framework, $ui, $params) {
    return array("display"=>"<div id=\"white_form\"><p>Media Streams has not been created yet.</p></div>");
}

function help($framework, $ui, $params) {
	return array("display"=>"<ul><li>Help menu not yet available.</li></ul>");
}

function galleries($framework, $ui, $params) {
	return array("display"=>"<div id=\"commnetivity_window\"><ul>panel not available</ul></div>");
}

function images($framework, $ui, $params) {
	$record = RECORD;
	if ( ACTION != "" ) {
		// use switch(ACTION) here.
		switch(ACTION) {
...
$extentions = $framework->get_extentions_from_mimetype("image/%");	
...
$javascript .= <<<EOF
	$('div[id=commnetivity_actions]').empty();
	$('div[id=commnetivity_actions]').html('<a class="prev">&laquo; Back</a><a class="next">More images &raquo;</a>');
	$(".scrollable").scrollable({ vertical: true, mousewheel: true });
	$('input[name=commnetivity_leftnav_search]').each(function() {
	    var default_value = this.value;
    	$(this).focus(function() {
...
$('div[id=commnetivity_preview]').show();
EOF;
...
function feeds($framework, $ui, $params) {
		$xhtml .= '<ul>';
...
function media_ajax_search($framework, $ui, $params) {
	return $results;
}
...
function images_cms_preview($framework, $ui, $params) {
